...
def builder_plus_built(id, other=None):
    if other is None:
        other = []
    out_div = f'{id}-out'
    layout = html.Div([
...
def register_callbacks(id):
    ...
    @callback(
        ...
        Input(components.archetype_builder.ArchetypeBuilderAIO.ids.store(id), 'data')
    )
    def update_children(data):
        tag_list = components.archetype_builder.create_archetype_tag_list(data)
        return tag_list


    @callback(
        Output(components.archetype_builder.ArchetypeBuilderAIO.ids.store(id), 'data', allow_duplicate=True),
        Input({'type': components.archetype_builder.remove_tag, 'index': ALL}, 'n_clicks'),
        State(components.archetype_builder.ArchetypeBuilderAIO.ids.store(id), 'data'),
